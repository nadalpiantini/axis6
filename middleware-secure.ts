...
import { applyEnhancedRateLimit, detectEndpointType } from '@/lib/security/production-rate-limit'
...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    const globalRateLimit = await applyEnhancedRateLimit(request, undefined, 'global_ip')
    ...
    const specificRateLimit = await applyEnhancedRateLimit(request, endpointType)
    ...
    if (!isPublicApi) {
      ...
      if (user) {
        const userRateLimit = await applyEnhancedRateLimit(request, 'api_general', user.id)
        if (userRateLimit) return userRateLimit
        
        // Extra strict rate limiting for sensitive endpoints
        if (isSensitiveApi) {
          const sensitiveRateLimit = await applyEnhancedRateLimit(request, 'sensitive', user.id)
  ...
  if (protectedRoutes.some(route => pathname.startsWith(route))) {
    ...
    if (user) {
      const userRateLimit = await applyEnhancedRateLimit(request, 'read', user.id)
      if (userRateLimit) return userRateLimit
    }
