...
export function ReflectionCard({ reflection, date }: ReflectionCardProps) {
      ...
      {isEditing ? (
        <div className="space-y-3">
          <textarea
            ...
            className={`w-full bg-white/10 border rounded-lg p-3 text-white placeholder-gray-400 resize-none h-20 ${
        ...
        <div className="space-y-3">
          {text ? (
            ...
            <p className="text-gray-500 italic">No reflection yet for today...</p>
          )}
          <button
            onClick={() => setIsEditing(true)}
            className="w-full py-2 px-4 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-medium transition-colors border border-white/20"
          >
