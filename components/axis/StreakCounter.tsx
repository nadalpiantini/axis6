...
export default function StreakCounter({ streaks, loading = false }: StreakCounterProps) {
  ...
  const getStreakStatus = (currentStreak: number) => {
    if (currentStreak === 0) return { label: 'No streak', emoji: '💤' }
    if (currentStreak < 3) return { label: 'Starting', emoji: '🌱' }
    if (currentStreak < 7) return { label: 'In progress', emoji: '🔥' }
    if (currentStreak < 30) return { label: 'Excellent!', emoji: '⚡' }
    if (currentStreak < 100) return { label: 'Incredible!', emoji: '🌟' }
