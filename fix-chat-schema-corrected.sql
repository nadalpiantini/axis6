...
-- Step 1: Check if the chat tables exist and create them if needed
DO $$
BEGIN
...
-- Step 4: Create default chat rooms - FIXED VERSION
-- Create default rooms using explicit category IDs instead of subqueries
DO $$
DECLARE
    physical_id INTEGER;
    mental_id INTEGER;
    emotional_id INTEGER;
    social_id INTEGER;
    spiritual_id INTEGER;
    material_id INTEGER;
BEGIN
    -- Get category IDs
    SELECT id INTO physical_id FROM axis6_categories WHERE slug = 'physical';
    SELECT id INTO mental_id FROM axis6_categories WHERE slug = 'mental';
    SELECT id INTO emotional_id FROM axis6_categories WHERE slug = 'emotional';
    SELECT id INTO social_id FROM axis6_categories WHERE slug = 'social';
    SELECT id INTO spiritual_id FROM axis6_categories WHERE slug = 'spiritual';
    SELECT id INTO material_id FROM axis6_categories WHERE slug = 'material';
    ...
    IF NOT EXISTS (SELECT 1 FROM axis6_chat_rooms WHERE name = 'Physical Wellness') THEN
        ...
        VALUES ('Physical Wellness', 'Share your fitness journey and healthy habits', 'category', physical_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM axis6_chat_rooms WHERE name = 'Mental Growth') THEN
        INSERT INTO axis6_chat_rooms (name, description, type, category_id) 
        VALUES ('Mental Growth', 'Discuss learning, productivity, and mental challenges', 'category', mental_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM axis6_chat_rooms WHERE name = 'Emotional Support') THEN
        INSERT INTO axis6_chat_rooms (name, description, type, category_id) 
        VALUES ('Emotional Support', 'A safe space for emotional well-being discussions', 'category', emotional_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM axis6_chat_rooms WHERE name = 'Social Connections') THEN
        INSERT INTO axis6_chat_rooms (name, description, type, category_id) 
        VALUES ('Social Connections', 'Build relationships and share social experiences', 'category', social_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM axis6_chat_rooms WHERE name = 'Spiritual Journey') THEN
        INSERT INTO axis6_chat_rooms (name, description, type, category_id) 
        VALUES ('Spiritual Journey', 'Explore mindfulness, meditation, and purpose', 'category', spiritual_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM axis6_chat_rooms WHERE name = 'Material Goals') THEN
        INSERT INTO axis6_chat_rooms (name, description, type, category_id) 
        VALUES ('Material Goals', 'Career, finances, and material aspirations', 'category', material_id);
    END IF;
